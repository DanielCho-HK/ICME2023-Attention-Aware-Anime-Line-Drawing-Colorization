...
from VGG import VGG
from SCFT_cbam import Generator_SCFT_cbam, Discriminator
...
log_dir = './logs_SCFT/'
checkpoint_dir = './train_SCFT/checkpoint/'
sample_dir = './train_SCFT/sample/'
...
def similarity_based_triple_loss(anchor, positive, negative, triplet_margin=12):
    distance = scaled_dot_product(anchor, positive) - scaled_dot_product(anchor, negative) + triplet_margin
    loss = torch.mean(torch.maximum(distance, torch.zeros_like(distance)))
    return loss
...
def train(opt, gen, dis, optim_g, optim_d):
    ...
    with tqdm(range(opt.iters)) as pbar:
        for i in pbar:
            ...
            # 训练生成器
            fake_img_gen, query, key, value = gen(sketch, reference_tps)
            fake_output = dis(torch.cat([fake_img_gen, sketch], dim=1))
            g_adv_loss = adv_loss(fake_output, torch.ones_like(fake_output))
            # g_l1_loss = l1_loss(fake_img_gen, reference) * 20
            g_l1_loss = l1_loss(fake_img_gen, reference) * 30
            g_triplet_loss = similarity_based_triple_loss(query, key, value)
            ...
            g_loss = g_l1_loss + g_triplet_loss + g_adv_loss + g_style_loss + g_vgg_loss
            writer.add_scalar("g_l1_loss", g_l1_loss.item(), global_step=i)
            writer.add_scalar("g_triplet_loss", g_triplet_loss.item(), global_step=i)
...
if __name__ == "__main__":
    ...
    generator = Generator_SCFT(sketch_channels=1, reference_channels=3)
    generator.to(device)
